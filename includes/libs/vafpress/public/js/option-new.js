;(function($) {
	...
	$('.vp-js-option-form').bind('submit', function(e) {
		...
		var updated_data = [];
		for ( var item in grt_data_object ) {
			var grt_value = grt_data_object[item];
			var _grt_value = ( Array.isArray( grt_value ) ) ? JSON.stringify(grt_value) : grt_value;
			
			var sml_value = sml_data_object[item];
			var _sml_value = ( Array.isArray( sml_value ) ) ? JSON.stringify(sml_value) : sml_value;
